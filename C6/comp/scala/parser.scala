...
private object ParserImp extends PARSER_SIG {
  ...
  class gapListener extends GAPBaseListener {
    ...
    override def exitForStm(ctx: GAPParser.ForStmContext): Unit = {
      ctx.ast =
        A.ForStm(
          A.SimpleVar(Symbol.symbol(ctx.ID().getText())),
          Exp(ctx.rngExp().ast),
          Stm(ctx.stms().ast))
    }
    ...
    override def exitVarExp(ctx: GAPParser.VarExpContext): Unit = {
      ctx.ast = A.VarExp(A.SimpleVar(Symbol.symbol(ctx.ID().getText())))
    }

    override def exitBoolExp(ctx: GAPParser.BoolExpContext): Unit = {
      ctx.ast = A.BoolExp(ctx.BOOL().getText() == "true")
    }

    override def exitIntExp(ctx: GAPParser.IntExpContext): Unit = {
      ctx.ast = A.IntExp(ctx.INT().getText().toInt)
    }

    override def exitStringExp(ctx: GAPParser.StringExpContext): Unit = {
      val s = ctx.STRING().getText()
      ctx.ast = A.StringExp(PP.parseString(s.substring(1, s.length-1), (s => ())))
    }

    override def exitRangeExp(ctx: GAPParser.RangeExpContext): Unit = {
      ctx.ast =
        A.RangeExp(
          A.IntExp(ctx.INT(0).getText().toInt),
          A.IntExp(ctx.INT(1).getText().toInt))
    }
  }
